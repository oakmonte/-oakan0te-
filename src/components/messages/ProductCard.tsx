...
export function ProductCard({ product, onAdd }: { product: SeedProduct; onAdd: () => void }) {
  return (
    <article className="w-[252px] overflow-hidden rounded-[18px] border border-chat-border bg-chat-elevated">
      <Link to="/store/products/$id" params={{ id: product.id }} aria-label={`View ${product.name}`}>
      ...
      <div className="p-3">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <Button asChild size="sm" variant="secondary" className="h-9 rounded-full bg-chat-soft text-chat-text active:scale-95">
            <Link to="/store/products/$id" params={{ id: product.id }}>View</Link>
          </Button>
          <Button type="button" size="sm" onClick={onAdd} className="h-9 rounded-full bg-chat-text text-chat-inverse active:scale-95">
            <ShoppingBag /> Add
          </Button>
        </div>
      </div>
