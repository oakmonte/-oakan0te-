      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-chat-muted">
            <span className="line-through">{offer.listPrice}</span> · {offer.expiresIn}
          </p>
        </div>
      </div>
      {settled ? (
        <p className="mt-3 border-t border-chat-border pt-3 text-center text-[12px] font-semibold text-chat-text">
          Offer {settled} · 08:31
        </p>
      ) : (
        <div className="mt-3 grid grid-cols-3 gap-1.5">
          {(["accepted", "countered", "declined"] as const).map((action) => (
            <Button
              ...
              onClick={() => setSettled(action)}
              className="h-9 rounded-full px-2 text-[11px] capitalize active:scale-95"
            >
