...
type PostPhoto = { id: string; media_url: string; thumbnail_url: string | null };

/** Full-screen picker over the seller's own published posts (photos only — a
...
export function PostImagePickerSheet({
  ...
  const [photos, setPhotos] = useState<PostPhoto[] | null>(null);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (cancelled) return;
        if (error) console.error("PostImagePickerSheet: failed to load posts", error);
        setPhotos(data ?? []);
      });
    return () => {
      cancelled = true;
