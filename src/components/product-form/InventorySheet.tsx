...
import { X, ChevronLeft, ScanBarcode, Check } from "lucide-react";
 ...
 *  from the store's Pickup locations screen; "Edit locations" here is
 *  selection-only. */
    ...
    <div className="fixed inset-0 z-50 bg-white flex flex-col min-h-dvh animate-in fade-in slide-in-from-bottom-6 duration-300 ease-out">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 flex flex-col gap-3">
        <input
          ...
          className="w-full text-base border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-gray-400 transition-colors duration-150"
        />
        <div className="relative">
          <input
            ...
            className="w-full text-base border border-gray-200 rounded-xl pl-4 pr-11 py-3.5 outline-none focus:border-gray-400 transition-colors duration-150"
          />
          <ScanBarcode
            size={18}
...
// Selection-only: picking a location here just tells this SKU where it's
// stocked. Creating or editing an actual location is deliberately not
// possible from here -- that only happens from the store's own Pickup
// locations screen, so there's exactly one place location data can drift.
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col min-h-dvh animate-in fade-in slide-in-from-bottom-6 duration-300 ease-out">
      <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-gray-100 px-4 h-14 flex items-center gap-3 shrink-0">
        ...
        <span className="font-semibold text-[15px] flex-1 text-center -ml-6">Edit locations</span>
      </div>

      <div className="flex-1 overflow-y-auto">
        {locations.length === 0 ? (
          <p className="px-4 py-8 text-sm text-gray-400 text-center">
            You haven't added any pickup locations yet. Add one from Pickup locations on your store
            home, then come back here to pick from them.
          </p>
        ) : (
          locations.map((loc) => {
