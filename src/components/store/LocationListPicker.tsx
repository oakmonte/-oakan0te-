    ...
    <div className="fixed inset-0 z-50 bg-sd-surface flex flex-col min-h-dvh animate-in fade-in slide-in-from-bottom-6 duration-[var(--duration-slow)] ease-[var(--ease-smooth-out)]">
      ...
      <div className="px-4 pt-3 pb-2 shrink-0">
        <input
          ...
          className="w-full bg-sd-soft rounded-lg px-3 py-2.5 text-base outline-none"
          autoFocus
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-8">
        {loadingNote && (
          <p className="px-4 py-2 text-xs text-sd-ink-faint border-b border-sd-line/50">
            {loadingNote}
          </p>
        )}
        {showCustomOption && (
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 border-b border-sd-line text-left bg-sd-elevated"
            type="button"
          >
        ...
        {filtered.length === 0 && !showCustomOption ? (
          <p className="px-4 py-6 text-sm text-sd-ink-faint text-center">Nothing found.</p>
        ) : (
          filtered.map((item) => (
            <button
              key={item.code}
              onClick={() => onSelect(item)}
              className="w-full flex items-center justify-between px-4 py-3 border-b border-sd-line/50 text-left"
              type="button"
            >
              <span className="text-[15px] text-sd-ink">{item.name}</span>
              <span className="w-4 h-4 rounded-full border border-sd-line shrink-0" />
            </button>
          ))
        )}
        {!search.trim() && items.length > BROWSE_CAP && (
          <p className="px-4 py-4 text-xs text-sd-ink-faint text-center">
